...
public class MethodCallHandlerImpl implements MethodCallHandler, StateProvider {
    ...
    private void initialize(int networkIgnoreMask, boolean forceSWCodec, List<String> forceSWCodecList, @Nullable ConstraintsMap androidAudioConfiguration) {
        if (mFactory != null) {
            return;
        }

        PeerConnectionFactory.initialize(InitializationOptions.builder(context).setEnableInternalTracer(true).createInitializationOptions());
        ...
        if (androidAudioConfiguration != null) {
            Integer usageType = AudioUtils.getAudioAttributesUsageTypeForString(androidAudioConfiguration.getString("androidAudioAttributesUsageType"));
            Integer contentType = AudioUtils.getAudioAttributesContentTypeFromString(androidAudioConfiguration.getString("androidAudioAttributesContentType"));
            ...
            if (usageType != null && contentType != null) {
                audioAttributes = new AudioAttributes.Builder().setUsage(usageType).setContentType(contentType).build();
            }
        }

        JavaAudioDeviceModule.Builder audioDeviceModuleBuilder = JavaAudioDeviceModule.builder(context).setUseHardwareAcousticEchoCanceler(true).setUseHardwareNoiseSuppressor(true).setSamplesReadyCallback(getUserMediaImpl.inputSamplesInterceptor);
        ...
        final PeerConnectionFactory.Builder factoryBuilder = PeerConnectionFactory.builder().setOptions(options);
        ...
        factoryBuilder.setVideoEncoderFactory(videoEncoderFactory).setVideoDecoderFactory(videoDecoderFactory);
        ...
        mFactory = factoryBuilder.setAudioDeviceModule(audioDeviceModule).createPeerConnectionFactory();
    ...
    public void onMethodCall(MethodCall call, @NonNull Result notSafeResult) {
        ...
        switch (call.method) {
            case "initialize": {
                ...
                if (constraintsMap.hasKey("networkIgnoreMask") && constraintsMap.getType("networkIgnoreMask") == ObjectType.Array) {
                ...
                if (constraintsMap.hasKey("forceSWCodec") && constraintsMap.getType("forceSWCodec") == ObjectType.Boolean) {
                    final boolean v = constraintsMap.getBoolean("forceSWCodec");
                    forceSWCodec = v;
                }
                List<String> forceSWCodecList = new ArrayList<>();
                if (constraintsMap.hasKey("forceSWCodecList") && constraintsMap.getType("forceSWCodecList") == ObjectType.Array) {
                ...
                if (constraintsMap.hasKey("androidAudioConfiguration") && constraintsMap.getType("androidAudioConfiguration") == ObjectType.Map) {
            ...
            case "setEnableFilter": {
                boolean enableFilter = call.argument("enableFilter");
                Log.d(TAG, "setEnableFilter: " + enableFilter);
                GPUImageBeautyFilter gpuImageBeautyFilter = getUserMediaImpl.getGpuImageBeautyFilter();
                if (gpuImageBeautyFilter != null) {
                    gpuImageBeautyFilter.setEnableFilter(enableFilter);
                }
                result.success(null);
                break;
            }
            case "setFilterParams": {
                float[] params = call.argument("params");
                Log.d(TAG, "setFilterParams: " + params[0] + " " + params[1] + " " + params[2]);
                GPUImageBeautyFilter gpuImageBeautyFilter = getUserMediaImpl.getGpuImageBeautyFilter();
                if (gpuImageBeautyFilter != null) {
                    gpuImageBeautyFilter.setFilterParams(params);
                }
                result.success(null);
                break;
            }
            case "setToneLevel": {
                double toneLevel = call.argument("toneLevel");
                Log.d(TAG, "toneLevel: " + toneLevel);
                GPUImageBeautyFilter gpuImageBeautyFilter = getUserMediaImpl.getGpuImageBeautyFilter();
                if (gpuImageBeautyFilter != null) {
                    gpuImageBeautyFilter.setToneLevel((float) toneLevel);
                }
                result.success(null);
                break;
            }
            case "setBeautyLevel": {
                double beautyLevel = call.argument("beautyLevel");
                Log.d(TAG, "setBeautyLevel: " + beautyLevel);
                GPUImageBeautyFilter gpuImageBeautyFilter = getUserMediaImpl.getGpuImageBeautyFilter();
                if (gpuImageBeautyFilter != null) {
                    gpuImageBeautyFilter.setBeautyLevel((float) beautyLevel);
                }
                result.success(null);
                break;
            }
            case "setBrightLevel": {
                double brightLevel = call.argument("brightLevel");
                Log.d(TAG, "setBrightLevel: " + brightLevel);
                GPUImageBeautyFilter gpuImageBeautyFilter = getUserMediaImpl.getGpuImageBeautyFilter();
                if (gpuImageBeautyFilter != null) {
                    gpuImageBeautyFilter.setBrightLevel((float) brightLevel);
                }
                result.success(null);
                break;
            }
            ...
            case "createVideoRenderer": {
                ...
                EventChannel eventChannel = new EventChannel(messenger, "FlutterWebRTC/Texture" + entry.id());
            ...
            case "startRecordToFile":
                ...
                try {
                    ...
                    if (call.hasArgument("audioChannel") && call.argument("audioChannel") != null) {
    ...
    private ConstraintsMap capabilitiestoMap(RtpCapabilities capabilities) {
        ...
        for (RtpCapabilities.CodecCapability codec : capabilities.codecs) {
            ...
            if (codec.numChannels != null) codecMap.putInt("channels", codec.numChannels);
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (iceServerMap.hasKey("url")) {
                if (hasUsernameAndCredential) {
                    iceServers.add(IceServer.builder(iceServerMap.getString("url")).setUsername(iceServerMap.getString("username")).setPassword(iceServerMap.getString("credential")).createIceServer());
                } else {
                ...
                switch (iceServerMap.getType("urls")) {
                    case String:
                        if (hasUsernameAndCredential) {
                            iceServers.add(IceServer.builder(iceServerMap.getString("urls")).setUsername(iceServerMap.getString("username")).setPassword(iceServerMap.getString("credential")).createIceServer());
                        } else {
                    ...
                    case Array:
                        ...
                        if (hasUsernameAndCredential) {
                            builder.setUsername(iceServerMap.getString("username")).setPassword(iceServerMap.getString("credential"));
                        }
    ...
    private RTCConfiguration parseRTCConfiguration(ConstraintsMap map) {
        ...
        // maxIPv6Networks
        if (map.hasKey("maxIPv6Networks") && map.getType("maxIPv6Networks") == ObjectType.Number) {
        ...
        if (map.hasKey("audioJitterBufferMaxPackets") && map.getType("audioJitterBufferMaxPackets") == ObjectType.Number) {
        ...
        if (map.hasKey("iceConnectionReceivingTimeout") && map.getType("iceConnectionReceivingTimeout") == ObjectType.Number) {
        ...
        if (map.hasKey("iceBackupCandidatePairPingInterval") && map.getType("iceBackupCandidatePairPingInterval") == ObjectType.Number) {
        ...
        if (map.hasKey("audioJitterBufferFastAccelerate") && map.getType("audioJitterBufferFastAccelerate") == ObjectType.Boolean) {
        ...
        if (map.hasKey("presumeWritableWhenFullyRelayed") && map.getType("presumeWritableWhenFullyRelayed") == ObjectType.Boolean) {
        ...
        if (map.hasKey("cryptoOptions") && map.getType("cryptoOptions") == ObjectType.Map) {
            ...
            conf.cryptoOptions = CryptoOptions.builder().setEnableGcmCryptoSuites(cryptoOptions.hasKey("enableGcmCryptoSuites") && cryptoOptions.getBoolean("enableGcmCryptoSuites")).setRequireFrameEncryption(cryptoOptions.hasKey("requireFrameEncryption") && cryptoOptions.getBoolean("requireFrameEncryption")).setEnableEncryptedRtpHeaderExtensions(cryptoOptions.hasKey("enableEncryptedRtpHeaderExtensions") && cryptoOptions.getBoolean("enableEncryptedRtpHeaderExtensions")).setEnableAes128Sha1_32CryptoCipher(cryptoOptions.hasKey("enableAes128Sha1_32CryptoCipher") && cryptoOptions.getBoolean("enableAes128Sha1_32CryptoCipher")).createCryptoOptions();
        }
        if (map.hasKey("enableCpuOveruseDetection") && map.getType("enableCpuOveruseDetection") == ObjectType.Boolean) {
    ...
    public String peerConnectionInit(ConstraintsMap configuration, ConstraintsMap constraints) {
        ...
        PeerConnection peerConnection = mFactory.createPeerConnection(conf, parseMediaConstraints(constraints), observer);
    ...
    public void getSources(Result result) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            ...
            for (int i = 0; i < devices.length; i++) {
                ...
                if (device.getType() == AudioDeviceInfo.TYPE_BUILTIN_MIC || device.getType() == AudioDeviceInfo.TYPE_BLUETOOTH_SCO || device.getType() == AudioDeviceInfo.TYPE_WIRED_HEADSET) {
    ...
    public void peerConnectionAddICECandidate(ConstraintsMap candidateMap, final String id, final Result result) {
        ...
        if (peerConnection != null) {
            ...
            IceCandidate candidate = new IceCandidate(candidateMap.getString("sdpMid"), sdpMLineIndex, candidateMap.getString("candidate"));
    ...
    public void dataChannelSend(String peerConnectionId, String dataChannelId, ByteBuffer bytebuffer, Boolean isBinary) {
        // Forward to PeerConnectionObserver which deals with DataChannels
        // because DataChannel is owned by PeerConnection.
        PeerConnectionObserver pco = mPeerConnectionObservers.get(peerConnectionId);
        if (pco == null || pco.getPeerConnection() == null) {
    ...
    public void addTransceiver(String peerConnectionId, String trackId, Map<String, Object> transceiverInit, Result result) {
        PeerConnectionObserver pco = mPeerConnectionObservers.get(peerConnectionId);
        MediaStreamTrack track = localTracks.get(trackId);
        if (track == null) {
    ...
    public void addTransceiverOfType(String peerConnectionId, String mediaType, Map<String, Object> transceiverInit, Result result) {
        PeerConnectionObserver pco = mPeerConnectionObservers.get(peerConnectionId);
        if (pco == null || pco.getPeerConnection() == null) {
    ...
    void requestPermissions(final ArrayList<String> permissions, final Callback successCallback, final Callback errorCallback) {
        ...
        PermissionUtils.requestPermissions(context, activity, permissions.toArray(new String[permissions.size()]), callback);
    }
}
